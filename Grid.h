...
class Grid {
  ...
  Cell *CellList[NumVerticalCells]
                [NumHorizontalCells]; // Every cell on the board

  GameObject *Clipboard; // Used in copy / cut / paste

public:
  Grid(Input *pIn, Output *pOut); // Allocates all Cells; does NOT create
                                  // Players (GameState does that)

  // ========== Board Operations ==========

  bool
  AddObjectToCell(GameObject *pNewObject); // Adds object to its cell; returns
                                           // false if cell is occupied
  void RemoveObjectFromCell(
      const CellPosition
          &pos); // Removes the GameObject of the Cell of the passed "position"
                 // Note: You may need to change the return type of this
                 // function (Think)
  ...
  void UpdatePlayerCell(
      Player *player,
      const CellPosition
          &newPosition); // Update the player's pCell with the CellList's Cell
                         // pointer of the "newPosition", Clears the player's
                         // circle from the previous cell and Draws it in the
                         // new cell
  ...
  int GetCellTypeCount(GameObjectType type); // Method to query for count of
                                             // cells of some type. //shahd
  ...
  // In Play mode, it only draws the player's info on the right side of the
  // toolbar
  void UpdateInterface(const GameState *pState) const;

  void PrintErrorMessage(string msg); // Shows msg on the status bar, waits for
                                      // a click, then clears it
