...
DeleteObjectAction::DeleteObjectAction(ApplicationManager *pApp)
    : Action(pApp) { // shahd
}
...
void DeleteObjectAction::Execute() {
  ...
  Grid *pGrid = pManager->GetGrid(); // gets grid of the obj that will be
                                     // deleted
  Input *pIn = pGrid->GetInput();
  Output *pOut = pGrid->GetOutput();
  ...
  // after obj deleted
  pManager->UpdateInterface(); // to update the interface 3ltol m3 el "output of
                               // successful msg"

  pOut->PrintMessage(
      "Game Object has deleted Successfully! Click anywhere to continue...");
  pIn->GetPointClicked(x, y);
  pOut->ClearStatusBar();
}